...
3. Adapter wraps in Cauce signal format
...
An Adapter connects a communication source to the Cauce Hub.
...
Cauce uses JSON-RPC 2.0 as the wire protocol.
...
|--------|-----------|-------------|
| `cauce.hello` | Client → Hub | Initial handshake, capability negotiation |
| `cauce.goodbye` | Client → Hub | Graceful disconnect |
| `cauce.ping` | Bidirectional | Keepalive |
| `cauce.pong` | Bidirectional | Keepalive response |

#### 4.2.2 Pub/Sub Methods

| Method | Direction | Description |
|--------|-----------|-------------|
| `cauce.publish` | Client → Hub | Publish message to topic |
| `cauce.subscribe` | Client → Hub | Subscribe to topic(s) |
| `cauce.unsubscribe` | Client → Hub | Unsubscribe from topic(s) |
| `cauce.signal` | Hub → Client | Deliver signal to subscriber |
| `cauce.ack` | Client → Hub | Acknowledge signal receipt |
...
| `cauce.subscription.revoke` | User → Hub | Revoke existing subscription |
...
{
  "method": "cauce.subscription.approve",
  "params": {
    "subscription_id": "sub_pending_001",
    "restrictions": {
...
{
  "method": "cauce.subscription.status",
  "params": {
    "subscription_id": "sub_pending_001",
    "status": "active",
    "topics": ["signal.email.*"],
...
```
wss://hub.example.com/cauce/v1/ws
```
...
```
GET /cauce/v1/sse?subscription_id=sub_abc123
...
event: signal
id: sig_1704288600_abc123
data: {"jsonrpc":"2.0","method":"cauce.signal","params":{...}}
...
```
GET /cauce/v1/poll?subscription_id=sub_abc123&last_id=sig_1704288500_xyz
Authorization: Bearer eyJhbG...
```
...
```
GET /cauce/v1/poll?subscription_id=sub_abc123&wait=true&timeout=30000
Authorization: Bearer eyJhbG...
```
...
```json
{
  ...
  "params": {
    ...
    "transport": "webhook",
    "webhook": {
      "url": "https://agent.example.com/cauce/webhook",
      "secret": "whsec_abc123",
      "headers": {
...
POST /cauce/webhook
Content-Type: application/json
X-Cauce-Signature: sha256=abc123...
X-Cauce-Timestamp: 1704288600
X-Cauce-Delivery-Id: del_xyz789
...
```
X-Cauce-Session-Id: sess_abc123
```
...
Cauce Hub exposes an A2A-compatible endpoint allowing external A2A agents to participate as first-class subscribers.

### 9.1 A2A Endpoint

```
https://hub.example.com/cauce/v1/a2a
```
...
```json
{
  ...
  "endpoints": {
    "a2a": "https://hub.example.com/cauce/v1/a2a",
    "sse": "https://hub.example.com/cauce/v1/sse"
  },
  "input_schema": {
    "type": "object",
    "properties": {
...
Cauce operations map to A2A tasks:

| Cauce Operation | A2A Task |
|------------------|----------|
...
1. A2A agent sends task to Cauce Hub endpoint
