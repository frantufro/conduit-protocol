...
Conduit supports five transport mechanisms. Implementations MUST support at least one. Hub implementations SHOULD support WebSocket and at least one HTTP-based transport for broad client compatibility.
...
Conduit Hub can expose an MCP (Model Context Protocol) interface, enabling MCP-compatible AI agents to interact with the Hub without implementing the native Conduit protocol. This lowers the barrier to adoption since many AI agents already speak MCP.
...
| MCP Primitive | Description | Conduit Mapping |
|---------------|-------------|-----------------|
...
```json
{
  "mcpServers": {
    "conduit": {
      ...
      "args": ["mcp-server"],
      "env": {
        "CONDUIT_HUB_URL": "https://hub.example.com",
...
Resources provide read-only access to Conduit data.
...
```json
{
  "uri": "conduit://signals/{subscription_id}",
...
{
  "name": "send_email",
  "description": "Send an email through Conduit",
  "arguments": [
    {
      "name": "to",
...
{
  "name": "send_sms",
  "description": "Send an SMS through Conduit",
  "arguments": [
    {
      "name": "to",
...
MCP with SSE provides near-parity with native Conduit for most use cases.
...
```python
# 1. Connect to Conduit Hub via MCP
mcp_client.connect("conduit-hub")
...
#### 11.2.2 Conduit Protocol Errors
